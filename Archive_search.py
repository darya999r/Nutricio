...
def archive_search():
    ...
    while True:
        ...
        if event == 'Открыть карту пациента':
            sg.popup(f"Вы переходите в карту пациента!{values['-ARCHIVE-'][0]}.\nЗакройте главное окно.")
            archive_pacient_name = ''.join(map(str, values['-ARCHIVE-']))
            print(archive_pacient_name)

    win_archive.close()
    return archive_pacient_name